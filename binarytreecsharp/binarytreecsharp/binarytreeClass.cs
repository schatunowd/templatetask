...
{
    class BinaryTreeClass<T> where T : IComparable<T>
    {
        private BinaryTreeClass <T> parent, left, right;
        private T val;
        private List<T> listForPrint = new List<T>();

        public BinaryTreeClass(T val, BinaryTreeClass<T> parent)
        {
            this.val = val;
            this.parent = parent;
        }
        ...
        public void Add(T val)
        {
            ...
            {
                if (this.left == null)
                    this.left = new BinaryTreeClass<T>(val, this);
                else
                      if (this.left != null)
                    this.left.Add(val);
            ...
            {
                if (this.right == null)
                    this.right = new BinaryTreeClass<T>(val, this);
                else
                    if (this.right != null)
                    this.right.Add(val);
            }
        ...
        private BinaryTreeClass<T> _search(BinaryTreeClass<T> tree, T val)
        {
            if (tree == null) 
                return null;
        ...
        public BinaryTreeClass<T> search(T val)
        {
        ...
        {
            BinaryTreeClass<T> tree = search(val);
            if (tree == null)
                return false;
            BinaryTreeClass<T> curTree;
            if (tree == this)
            {
                if (tree.right != null)
                    curTree = tree.right;
        ...
        private void _print(BinaryTreeClass<T> node)
        {
            if (node == null) 
                return;
            _print(node.left);
